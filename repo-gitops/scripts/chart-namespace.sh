#!/usr/bin/env bash
#
# Print the namespace a chart under charts/ declares for itself.
#
...
chart="$1"
# Beside this script, one level up, so a clone of this repo on its own resolves
# the same. From the script's location and not $PWD — make calls it from the
# tree above and the Makefile is free to move without this line following.
repo="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
values="$repo/charts/$chart/values.yaml"
